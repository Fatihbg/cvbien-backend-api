...
class HRLine(Flowable):
    ...
    def __init__(self, width=None, thickness=0.5, color=black):
...
app = FastAPI(title="CV Optimizer API V15 - Content Integrity Lock")
...
def extract_text_from_pdf(pdf_path: str) -> str:
    """Utilise LlamaIndex pour extraire le texte du PDF."""
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Erreur LlamaIndex (lecture PDF): {str(e)}")
    ...
    try:
        ...
        for line in lines:
            ...
            if (len(line) < 45 and line.isupper()) or (len(line) < 60 and line.endswith(':')):
                ...
                story.append(HRLine(thickness=0.8, color=HexColor('#003366')))
...
async def optimize_cv(cv_file: UploadFile = File(...), job_offer: str = Form(...)):
    """
    Endpoint principal qui prend un fichier PDF et une offre d'emploi,
    ...
    try:
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=cv_optimise_{cv_file.filename.replace('.pdf', '').replace('.txt', '')}.pdf"}
        )
...
@app.get("/")
async def root():
    return {"message": "CV Optimizer API 🚀 - V15 (Content Integrity Lock)"}
